<template>
  <div @drop="dragover = false; handleDropFiles($event)" @dragenter="dragover = true" @dragover="dragover = true" @dragleave = "dragover = false" :class="{ 'drag-over': dragover }">
      <div class="text-area">
        <template v-if="texts.length > 0 || !!intermediateText">
          <p v-for="(text, index) in texts" :key="index">{{ text }}<span v-if="textsProgressing" class="hint">...</span></p>
          <p v-if="intermediateText">{{ intermediateText }}</p>
        </template>
        <template v-if="textsLoading">
          <p>
            <span class="hint">Loading...</span>
          </p>
        </template>
        <template v-if="!(texts.length || intermediateText) && !textsLoading">
          <p>
      ...
      <div class="toolbar">
        ...
        <div class="upload-audio">
          <input type="file" id="upload" @change="handleSelectedFiles" hidden/>
      ...
      <div v-if="dragover" class="drop-hint"><span>Drop here...</span></div>
  </div>
</template>
...
export default defineComponent({
  ...
  data() {
    ...
    const reloadSessionTimeout: any = 0;
    return {
      recording: false,
      connected: false,
      audioContext,
  ...
  methods: {
   async handleSessionChanged() {
    ...
    async reloadSession() {
      ...
      model.forEach((m: any) => {
        ...
        if (m.status && m.status !== 'COMPLETED') {
          clearTimeout(this.reloadSessionTimeout);
          this.reloadSessionTimeout = setTimeout(this.reloadSession, 2000);
          this.textsProgressing = true;
        }
      });
    },
    ...
    handleDropFiles($event: any) {
      if ($event.dataTransfer.files.length === 1) {
        this.uploadFile($event.dataTransfer.files[0]);
      } else {
        alert('Unexpected number of files ' + $event.dataTransfer.files.length);
      }
    },
    handleSelectedFiles($event: any) {
      if ($event.target.files.length === 1) {
        ...
        alert('Unexpected number of files ' + $event.target.files.length);
      }
    },
    async uploadFile(file: File) {
      ...
      const result = await makeRequest('POST', '/transcribe-job', formData);
      const model = JSON.parse(result);
      this.$emit('openSession', model.sessionId);
    }
  }
});
</script>
